...
if(gen_pythia8) {    
    ...
    pythia8->set_config_file("phpythia8_PionToMuons.cfg"); // Ensure forced decay to muons

    if(SQ_vtx_gen) {
        pythia8->enableLegacyVtxGen();
    } else {
    ...
    // Apply AND trigger to ensure conditions are met
    //pythia8->set_trigger_AND();
    pythia8->set_trigger_OR();

    // Single muon triggers
    PHPy8ParticleTrigger* trigger_mu = new PHPy8ParticleTrigger();
    trigger_mu->AddParticles("13");   // Muon (positive and negative)
    trigger_mu->AddParticles("-13"); 
    trigger_mu->SetPxHighLow(-2.0, 2.0);  // Set transverse momentum range (adjust as needed)
    trigger_mu->SetPyHighLow(-2.0, 2.0);  // Set transverse momentum range (adjust as needed)
    trigger_mu->SetPzHighLow(120, 30);    // Longitudinal momentum range (e.g., 30 < p_z < 120 GeV)
    pythia8->register_trigger(trigger_mu);
}
